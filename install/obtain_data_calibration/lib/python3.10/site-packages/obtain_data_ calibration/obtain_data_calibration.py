...
class DetectionAstra(Node):
    def __init__(self):
        ...
        self.image_sub_body = self.create_subscription(PointCloud2, '/processed/points/body_c2', self.data_camera2, 10)
        
        # Publicador
        #self.publisher = self.create_publisher(PointCloud2, '/processed/points', 10)
        self.publisher_body = self.create_publisher(PointCloud2, '/processed/points/body_c2_calibrated', 10)
    ...
    def data_camera2(self, msg):
        # Read points with their id and visibility
        points = list(pc2.read_points(msg, skip_nans=True, field_names=("id", "x", "y", "z", "visibility")))

        processed_points = []

        for point in points:
            id, x, y, z, visibility = point

            # Add 1 to the point for multiplication with the 4x4 matrix
            point_with_one = np.array([x, y, z, 1.0])

            # Multiply by the calibration matrix
            transformed_point = np.dot(self.k, point_with_one)

            # Normalize the point
            transformed_point /= transformed_point[3]

            # Append the id and visibility back to the transformed coordinates
            processed_points.append([id, transformed_point[0], transformed_point[1], transformed_point[2], visibility])

        # Create a new PointCloud2 message
        header = msg.header
        fields = [
            PointField(name='id', offset=0, datatype=PointField.UINT32, count=1),
            ...
            PointField(name='visibility', offset=16, datatype=PointField.FLOAT32, count=1)
        ]
        
        # Convert processed points to a structured numpy array
        structured_points = np.array(processed_points, dtype=[('id', np.uint32), ('x', np.float32), ('y', np.float32), ('z', np.float32), ('visibility', np.float32)])

        # Create and publish the transformed PointCloud2 message
        cloud_msg = pc2.create_cloud(header, fields, structured_points)
