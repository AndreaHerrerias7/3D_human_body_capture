...
class DetectionAruco(Node):
    ...
    def publisher_image(self):
        ...
        if self.observations[0] and self.observations[1]:
            ...
            self.get_logger().info(f'TIEMPO1, {self.observations[0].tstamp}')
            self.get_logger().info(f'TIEMPO2, {self.observations[1].tstamp}')

            self.get_logger().info(f'TIEMPO1, {self.observations[0].tstamp.sec}')
            self.get_logger().info(f'TIEMPO2, {self.observations[1].tstamp.nanosec}')
            ...
            if abs(self.observations[0].tstamp.sec - self.observations[1].tstamp.sec) < TH_TSTAMPS:
                ...
                self.get_logger().info(f'AAAAAAA')
                self.get_logger().info(f'AAAAAAA')
                self.get_logger().info(f'AAAAAAA')
                self.get_logger().info(f'AAAAAAA')
                self.get_logger().info(f'AAAAAAA')
                ...
                if len(msg.correspondences) > 0:
                    self.get_logger().info(f'PlaneMarch, {msg.correspondences}')
